        ...
        if context.status == "connected" then
            if self.expanded_server == context.name then
                ...
                for _, tracked in ipairs(self.interactive_lines) do
                    ...
                    elseif tracked.type == "tool" or tracked.type == "resource" or tracked.type == "resourceTemplate" then
...
local function render_cap_section(items, title, server_name, type, current_line)
    ...
    table.insert(lines, Text.pad_line(
        NuiLine():append(" " .. icons[type] .. " " .. title .. ": ", Text.highlights.muted), nil, 4))
...
function MainView:render_servers(line_offset)
    ...
    for _, server in ipairs(sorted_servers) do
        ...
        if server.status == "connected" and server.capabilities and self.expanded_server == server.name then
            if #server.capabilities.tools + #server.capabilities.resources + #server.capabilities.resourceTemplates == 0 then
                table.insert(lines, Text.pad_line(NuiLine():append("No capabilities available", Text.highlights.muted),
                    nil, 6))
                table.insert(lines, Text.empty_line())
            ...
            if #server.capabilities.tools > 0 then
                local section_lines, new_line, mappings = render_cap_section(server.capabilities.tools, "Tools",
                    server.name, "tool", current_line)
            ...
            if #server.capabilities.resources > 0 then
                local section_lines, new_line, mappings = render_cap_section(server.capabilities.resources, "Resources",
                    server.name, "resource", current_line)
                vim.list_extend(lines, section_lines)
                for _, m in ipairs(mappings) do
                    self:track_line(m.line, m.type, m.context)
                end
            ...
            if #server.capabilities.resourceTemplates > 0 then
                local section_lines, new_line, mappings = render_cap_section(server.capabilities.resourceTemplates,
                    "Resource Templates", server.name, "resourceTemplate", current_line)
                vim.list_extend(lines, section_lines)
                for _, m in ipairs(mappings) do
                    self:track_line(m.line, m.type, m.context)
                end
...
function MainView:after_enter()
    ...
    if self.active_capability then
        ...
        if self.cursor_positions.browse_mode then
            local new_pos = {math.min(self.cursor_positions.browse_mode[1], line_count),
...
function MainView:render()
    ...
    if self.active_capability then
        ...
        local breadcrumb = NuiLine():append(self.active_capability.server_name, Text.highlights.muted):append(" > ",
            Text.highlights.muted):append(" " .. self.active_capability.info.name .. " ", Text.highlights.info)
